...
function requestBySearchBar(searchText) {
	...
	if (searchText.length >= 3) {
		...
		var regex = new RegExp(normalizeElement(`${searchText}`));
			console.log(regex)
			...
			for (let key in recipes[i].ingredients) {
				// console.log(key)
				let ingredientElts = recipes[i].ingredients[key].ingredient;
				ingredientsArray.push(ingredientElts);
			}
			// console.log(ingredientsArray)
			if(
				normalizeElement(recipes[i].name).match(regex) ||
				normalizeElement(`${ingredientsArray}`).match(regex) ||
				normalizeElement(recipes[i].description).match(regex)
			){
				res.push(recipes[i])
				console.log(res)
			}
